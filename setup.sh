...
print_summary() {
  ...
  echo -e "  ${BOLD}Credentials:${NC}  $CRED_FILE (AES-256-GCM encrypted)"
  echo -e "  ${BOLD}Device key:${NC}   ~/.corben-mcp/device.key (ECDSA P-256, encrypted)"
  echo ""
  echo -e "  ${BOLD}Security:${NC}"
  echo "    - API key encrypted with hardware-bound machine ID"
  echo "    - ECDSA keypair for challenge-response auth (no secrets over wire)"
  echo "    - Session tokens: 15 min TTL, locked to your IP address"
  echo "    - IP change = instant invalidation + automatic re-auth"
  echo ""
  echo -e "  ${BOLD}220+ tools:${NC} sites, email, CRM, invoicing, database,"
  echo "  calendar, phone/SMS, automation, AI agents, and more"
  echo ""
  echo "  Just ask your AI agent things like:"
  echo ""
  ...
  echo "    Devices:    Dashboard → Settings → MCP Devices"
  echo "    Uninstall:  rm -rf ~/.corben-mcp"
  echo ""
}
